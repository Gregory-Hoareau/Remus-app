...
export class SimulateurPage implements OnInit {
  ...
  constructor(
    ...
    public diceHistoryService: DiceHistoryService,
  ...
  async specialDicesAlert() {
    ...
    const alert = await this.alertCtrl.create({
      header:
        "Sélectionner le JDR dont vous souhaitez utiliser les dés spéciaux :",
      inputs: tab,
      buttons: [
        {
          text: "Valider",
          handler: (data) => {
            this.diceService.resetDices();
            if (
              this.diceService.specialGame.get(data) === this.diceService.Normal
            ) {
      ...
      buttons: [
        {
          ...
          handler: (data) => {
            if (data.macroName !== "") {
              this.macroService.createMacro(
                data.macroName,
                this.diceService.diceSelected,
                this.diceService.StringifyDice(),
                this.diceService.modifier,
                this.diceService.normalDices
              );
  ...
  async presentAlertConfirmSpecial(data: string[]) {
    ...
    for (const path of data) {
      message =
        message +
        '<ion-img class="resultPopupDices" src=' +
        path +
        "></ion-img>";
    }
  ...
  async noDiceAlert() {
    const alert = await this.alertController.create({
      ...
      message:
        "Vous devez choisir au moins un dé pour pouvoir faire une lancé de dé",
      buttons: ["Compris"],
    });
    await alert.present();
  }
  ...
  macroLaunch(macro: Macro) {
    this.presentAlertConfirm(
      this.diceService.launchDice(macro.dices, macro.modifier)
    );
  }
}
