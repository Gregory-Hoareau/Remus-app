...
export class SessionChatPage implements OnInit {
  ...
  constructor(
    ...
    public playerServ: PlayersService,
  ...
  async send() {
    ...
    if (!this.playerServ.isHost)
      this.playerServ
        .getPlayerByName("host")
        .conn.send({ message: message, target: this.target.name });
    this.playerServ
      .getConv(this.target)
      .addMessage(
        new Message(new Date(), this.playerServ.me(), message, this.target)
      );
    setTimeout(() => {
  ...
  getFilteredConv(): Conversation {
    ...
    this.playerServ.playersList.forEach((player) => {
      ...
      if (this.playerServ.getConv(player))
        filteredConv = filteredConv.concat(
          this.playerServ.getConv(player)
        ) as Conversation;
    });
    filteredConv = filteredConv.sort(
      (a, b) => a.timestamp.getTime() - b.timestamp.getTime()
    );
    //console.log("Conversations : ", filteredConv);
    return filteredConv.filter(
      (message) =>
        message.target == this.target /*&&  message.player==this.player*/ ||
        /*message.target==this.player &&*/ message.player == this.target
    ) as Conversation;
  }
